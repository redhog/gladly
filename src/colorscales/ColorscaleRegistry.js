...
export function buildColorGlsl() {
  ...
  parts.push('  if (cs_a >= 0) {')
  parts.push('    if (!a_nan) {')
  parts.push('      vec4 ca = gladly_map_color_raw(cs_a, range_a, v_a, type_a);')
  parts.push('      return gladly_apply_color(ca);')
  parts.push('    }')
  parts.push('    if (!b_nan) {')
  parts.push('      vec4 cb = gladly_map_color_raw(cs_b, range_b, v_b, type_b);')
  parts.push('      return gladly_apply_color(cb);')
  parts.push('    }')
  parts.push('    return gladly_apply_color(vec4(0.0));')
  parts.push('  }')
  ...
  parts.push('    vec4 ca = gladly_map_color_raw(cs_a, range_a, v_a, type_a);')
  parts.push('    vec4 cb = gladly_map_color_raw(cs_b, range_b, v_b, type_b);')
  parts.push('    return gladly_apply_color((ca + cb) / 2.0);')
  parts.push('  }')
  parts.push('  if (!a_nan) {')
  parts.push('    vec4 ca = gladly_map_color_raw(cs_a, range_a, v_a, type_a);')
  parts.push('    return gladly_apply_color(ca);')
